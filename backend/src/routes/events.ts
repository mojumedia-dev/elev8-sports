...
router.get('/', authenticate, async (req: Request, res: Response) => {
  ...
  const where = teamId ? { teamId: teamId as string } : { team: { members: { some: { userId: req.user!.userId } } } };
...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  const event = await prisma.event.findUnique({ where: { id: req.params.id }, include: { team: true, rsvps: { include: { user: { select: { firstName: true, lastName: true } } } } } });
