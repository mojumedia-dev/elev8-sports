...
router.get('/:id', authenticate, async (req: Request, res: Response) => {
  const org = await prisma.organization.findUnique({ where: { id: req.params.id }, include: { teams: true, admin: { select: { firstName: true, lastName: true, email: true } } } });
...
router.put('/:id', authenticate, requireRole('ORG_ADMIN'), async (req: Request, res: Response) => {
  const { name, description, logoUrl, website } = req.body;
  const org = await prisma.organization.updateMany({ where: { id: req.params.id, adminId: req.user!.userId }, data: { name, description, logoUrl, website } });
...
router.delete('/:id', authenticate, requireRole('ORG_ADMIN'), async (req: Request, res: Response) => {
  await prisma.organization.deleteMany({ where: { id: req.params.id, adminId: req.user!.userId } });
  res.json({ success: true });
});
